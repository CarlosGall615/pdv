...
class DashboardView(ctk.CTkFrame): 
    ...
    def __init__(self, master, controller, **kwargs): # Agora recebe controller
        ...
        for p in ["Dia", "Semana", "Mês"]:
            ctk.CTkButton(self.frame_filtros, text=p, width=100, height=32,
                          ...
                          # Aqui o botão avisa o controller que o filtro mudou
                          command=lambda opt=p: self.controller.filtrar_dashboard(opt)).pack(side="left", padx=10, pady=10)
    ...
    def renderizar_grafico(self, dados, periodo):
        ...
        # Dentro de renderizar_grafico no dashboard_view.py
        else:
            eixo_x_bruto = [datetime.strptime(d['data'], "%d/%m/%Y").strftime("%d/%m") for d in dados]
